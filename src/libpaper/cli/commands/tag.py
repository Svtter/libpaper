...
@tag_commands.command()
@click.argument("name", type=str)
@click.option("--description", "-d", help="标签描述")
@click.option("--color", "-c", help="标签颜色 (hex 格式，如 #ff0000)")
@click.pass_context
def create(
    ctx: click.Context, name: str, description: Optional[str], color: Optional[str]
):
    """创建新标签"""
...
@click.pass_context
def list(ctx: click.Context, popular: bool, unused: bool, limit: int):
    """列出标签"""
...
def cleanup(ctx: click.Context):
    ...
    async def _cleanup():
        ...
        try:
            ...
            if deleted_count > 0:
                console.print(
                    f"[green]✓ 清理完成，删除了 {deleted_count} 个未使用的标签[/green]"
                )
            else:
