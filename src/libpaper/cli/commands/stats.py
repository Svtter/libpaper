...
def overview(ctx: click.Context):
    ...
    async def _overview():
        ...
        try:
            ...
            # 文献统计
            papers_table = Table(
                title="文献统计", show_header=True, header_style="bold magenta"
            )
            ...
            papers_table.add_row(
                "平均文件大小", format_size(storage_stats["average_size"])
            )
            papers_table.add_row(
                "有摘要的文献", str(storage_stats["papers_with_abstract"])
            )
            ...
            # 分类和标签统计
            org_table = Table(
                title="组织结构", show_header=True, header_style="bold magenta"
            )
            ...
            # 存储统计
            storage_table = Table(
                title="存储信息", show_header=True, header_style="bold magenta"
            )
    ...
    async def _storage():
        ...
        try:
            ...
            if storage_stats["total_papers"] > 0:
                abstract_percent = (
                    storage_stats["papers_with_abstract"]
                    / storage_stats["total_papers"]
                ) * 100
                doi_percent = (
                    storage_stats["papers_with_doi"] / storage_stats["total_papers"]
                ) * 100
    ...
    async def _tags():
        ...
        try:
            ...
            if popular_tags:
                # 热门标签表格
                table = Table(
                    title=f"热门标签 (前 {len(popular_tags)} 个)",
                    show_header=True,
                ...
                for i, tag in enumerate(popular_tags, 1):
                    ...
                    description = (
                        tag.description[:30] + "..."
                        if tag.description and len(tag.description) > 30
        ...
        try:
            ...
            if collections:
                ...
                sorted_collections = sorted(
                    collections, key=lambda c: c.paper_count, reverse=True
                )
