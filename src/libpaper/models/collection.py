...
class Collection(SQLModel, table=True):
    ...
    id: UUID = Field(default_factory=uuid4, primary_key=True, description="分类唯一标识")
    name: str = Field(..., min_length=1, max_length=100, description="分类名称")
    description: Optional[str] = Field(None, max_length=500, description="分类描述")
    parent_id: Optional[UUID] = Field(None, foreign_key="collections.id", description="父分类ID，支持层次化结构")
    ...
    parent: Optional["Collection"] = Relationship(
